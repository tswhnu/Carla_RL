...
def speed_reward(current_speed, speed_limit, ratio1, ratio2):
    ...
    if speed_limit == 0:
        ...
        reward = - (current_speed - speed_limit) ** 2
    else:
...
class World(object):
    ...
    def restart(self):
        ...
        walker_transform = carla.Transform(carla.Location(x=15, y=28.4, z=0.5), carla.Rotation(yaw=0))
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(carla.Location(x=-10, y=28.4, z=0.5), carla.Rotation(yaw=0))
            self.model3 = self.world.get_blueprint_library().filter("model3")[0]
            self.player = self.world.try_spawn_actor(self.model3, spawn_point)
            self.modify_vehicle_physics(self.player)
        # self.player.set_velocity(carla.Vector3D(30, 0.0, 0.0))
...
def game_loop(args):
    ...
    last_vel = None
    position_hist = []
    walker_position = [15, 28.4]
    destination = [walker_position[0] + 5.0, walker_position[1]]
    epi_reward = 0
    try:
        ...
        while True:
            ...
            vehicle_speed = world.player.get_velocity()
            ...
            current_location = [world.player.get_location().x, world.player.get_location().y]
            if last_vel is None:
                vel_hist = vehicle_vel
            else:
                vel_hist = last_vel
            if len(position_hist) == 0:
                last_location = current_location
            else:
                last_location = position_hist[-1]
            position_hist.append(current_location)
            ped_location = [15, 28.4]
            Lt = math.sqrt(
                (last_location[0] - destination[0]) ** 2 + (last_location[1] - destination[1]) ** 2)
            Lt1 = math.sqrt(
                (current_location[0] - destination[0]) ** 2 + (current_location[1] - destination[1]) ** 2)
            des_ped = math.sqrt(
