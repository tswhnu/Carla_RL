...
class SENSOR(object):
    ...
    def actor_detection(self, local_actor):
        ...
        # global position of the actor that need to be detected
        actor_loc = local_actor.get_transform().location

        return self.bounding_box.contains(actor_loc, vehicle_transform)
    ...
    def state_output(self, local_actor):
        location, velocity, heading_direction = self.info_output(local_actor)
...
try:
    ...
    while True:
        if sensor_box.actor_detection(walker):
            print("contact risk")
            state = sensor_box.state_output(walker)
            print(state.shape)
